...
function inferEventType(eventName) {
  ...
  if (eventName.includes('scan') || eventName.includes('finding')) return 'product'
  return 'unknown'
}
...
function normalizeEvent(rawEvent, { siteKey, requestUrl }) {
  ...
  if (metadataJson.length > maxMetadataLength) metadataJson = metadataJson.slice(0, maxMetadataLength)
...
function normalizeRequestBodyToEvents(body) {
  ...
  return [
    {
      id: body.id,
      eventName: body.eventName ?? body.name ?? body.event ?? body.type ?? 'unknown_event',
...
function resolveStage(event) {
  ...
  if (name === 'scan_demo_started' || name === 'scan_demo_completed') return 'product_used'
  if (name === 'plan_selected' || name === 'billing_selected') return 'plan_selected'
  if (name === 'checkout_requested' || name === 'checkout_started') return 'checkout_started'
  if (name === 'checkout_opened' || name === 'checkout_popup_opened') return 'checkout_redirected'
  if (name === 'checkout_error' || name === 'checkout_popup_blocked') return 'checkout_start_failed'
  if (name === 'payment_success_landing' || name === 'checkout_success_return') return 'payment_completed'
  if (name === 'cta_click' || name === 'ui_click' || name === 'pricing_intent') return 'launch_clicked'
  return 'unknown'
}
...
async function ingestEvents(db, rawEvents, options) {
  ...
  for (const event of events) {
    const existingEvent = await db.prepare('SELECT id FROM analytics_events WHERE id = ?').bind(event.id).first()
    ...
    if ((result.meta?.changes ?? result.changes ?? 0) > 0) ingested++
  }
...
export async function handleAnalyticsRequest(request, env, { siteKey = 'syntheticuserlab' } = {}) {
